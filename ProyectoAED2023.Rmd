...
check_na_procedence <- function(raw_data_df, data_df) {
  ...
  if ( any(colnames(raw_data_df) != colnames(data_df)) ) {
    stop("check_na_procedence: Both data.frame objects must have the same variables.")
  }
  ...
  if (length(vars_with_na) < 1) {
    ...
    print("check_na_procedence: No NAs introduced in data.frame.")
    return(NULL)
    
  } else if (length(vars_with_na) < length(colnames(data_df))) {
    
    print("check_na_procedence: Ommiting values with 0 NAs in summary table.")
  }
  ...
  for (var_name in na_df$variable) {
    ...
    if ( all_spaces_or_tabs ) {
      ...
      message <- "All NAs correspond to empty entries."
      
    } else {
      
      message <- paste("Lost values:",
                           paste(lost_values, collapse = ", "), sep = " ")
    }
