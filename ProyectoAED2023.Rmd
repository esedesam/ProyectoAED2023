...
pacman::p_load(kableExtra, readxl, stringr, tidyr, dplyr, ggmap, leaflet, leaflet.extras, leaflet.extras2, sf, colorspace, car, fitdistrplus, ggmosaic)
```
...
barplot(var_com, names.arg = names(var_com), cex.names = 0.5, las=2, main = "Diferencia entre Altas y Bajas", xlab = "Categorías", ylab = "Diferencia")
```

A continuación, estudiaremos el flujo migratorio según el tamaño de los municipios para valorar el éxodo rural.
...
Debido a que en el conjunto de datos actual, únicamente poseemos una variable numérica, decidimos recuperar la variable ANONAc, la cual debería tener una gran correlación con la variable EDAD.
```{r}
# Si estás utilizando el paquete dplyr y el conjunto de datos se llama "datos"
library(dplyr)
bivar <- data %>%
  select(ANONAC, EDAD)
...
Por esta razón, ambas correlaciones son iguales, ya que la relación entre ambas variables es lineal.

### Nméricas- Categóricas
...
ggplot(mosaic_df) +
  ...
  geom_mosaic_text(
    ...
    as.label = TRUE) +
  labs(
    title = "Número de variaciones residenciales en función de TAMUALTA y TAMUBAJA",
    x = "TAMUBAJA",
    y = "TAMUALTA") +
  theme_minimal() +
  guides(fill = "none")
...
Respecto a los valores NA, hemos decidido no imputar ningún valor faltante, ya que consideramos que nuestra muestra es suficientemente grande como para poder trabajar sin dichos valores y tampoco queremos introducir datos que modifiquen las características de nuestro conjunto de datos.
