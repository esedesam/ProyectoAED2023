...
Por otra parte, creamos nuevas variables que contienen las comunidades autónomas, para poder analizar también las variaciones residenciales entre ellas. Para ello, aprovechamos el diccionario de municipios, ya que en él también aparecen codificadas las comunidades autónomas. Dado que las variables de municipios presentan muchos NAs, intentamos rellenar las comunidades autónomas buscando coincidencias de la provincia.
...
comu_dict <- municipios_data[c("CODAUTO", "CPRO", "CMUN")]
comu_dict <- comu_dict %>%
      unite(CMUN_complete, CPRO, CMUN, sep = "")

for (sufix in var_sufixes) {
  
  data_df[paste0("COMU", sufix)] <- as.character(factor(
    x = raw_data_df[[paste0("MUNI", sufix)]],
    levels = comu_dict$CMUN_complete,
    labels = comu_dict$CODAUTO))
  
  data_df[
    data_df[[paste0("PROV", sufix)]] == "Extranjero",
    paste0("COMU", sufix)] <- "Extranjero"
}
```
