...
main_sheet <- "Diseño"
vars_table <- read_excel(excel_file_path, sheet = "Diseño",
...
for (sheet_name in sheet_names[sheet_names != "Diseño"]) {
  sheet_data <- read_excel(excel_file_path, sheet = sheet_name,
                           col_names = FALSE)
  sheet_list[[sheet_name]] <- sheet_data
}

municipios_file <- "./data/diccionario21.xlsx"
relacion_municipios <- read_excel(municipios_file, skip = 1, col_names = TRUE)
```
...
for (var_idx in vars_with_dict) {
  ...
  # Cogemos la hoja y buscamos el comienzo del diccionario
  aux_table <- sheet_list[[dict_sheet]]
  dict_pos <- which(aux_table == dict_name, arr.ind = TRUE)
  
  # Buscamos donde acaba el diccionario
  ...
  # Sustituimos los códigos por su valor segun el diccionario
  values <- dict_df$Descripción[match(raw_data_df[[vars_table$Variable[var_idx]]], dict_df$Código)]
  data_df[vars_table$Variable[var_idx]] <- values
  # Probar con factors levels y labels en vez de match
}
```

```{r}
# Interpretación las variables de municipios

dict_municipios <- sheet_list$`Anexo - Lista de países`[-4:-1,1:2] # mejorable
colnames(dict_municipios) <- dict_municipios[1,]
dict_municipios <- dict_municipios[-1,]
relacion_municipios <- relacion_municipios[c("CMUN", "NOMBRE")]
colnames(relacion_municipios) <- colnames(dict_municipios) # arreglar con orden
dict_municipios <- rbind(dict_municipios, relacion_municipios)

vars_municipio <- which(!is.na(vars_table$Observaciones))

for (var_idx in vars_municipio) {
  values <- dict_municipios$Descripción[match(raw_data_df[[vars_table$Variable[var_idx]]], dict_municipios$Código)]
  data_df[vars_table$Variable[var_idx]] <- values
}
```

```{r}
unique(raw_data_df$MUNIALTA[is.na(data_df$MUNIALTA)])
